...
Instantiate polynomial from its coefficients or from given point set using interpolation.
Find it's root but beware that it may give more roots to the left or to the right
...
Finds $x:\; x \equiv a \pmod{m},\; x \equiv b \pmod{n}$.
If $|a| < m,\; |b| < n \implies 0 \le x < \text{lcm}(m,n)$.
Assumes $mn < 2^{62}$.
\tc{\log n}
...
\vspace{-0.2em}
$a^{-1} \pmod{m}$ is s.t $a \cdot (a^{-1} \pmod{m}) \equiv 1 \pmod{m}$. $a^{-1}$ exists $\iff a, m$ are coprime ($\gcd{a,m} = 1)$)
...
\cppcode{math/modulo/modinv.cpp}
...
Eratosthenes sieve.
Note that \verb|S = (int)round(sqrt(LIM))|
...
Given two numbers $m,n$: $m > n > 0$, $m$ and $n$ are coprime, $m - n$ is odd, generate triple: $a = m^2 - n^2, b = 2mn, c = m^2 + n^2$.
In such way triples are unique.
When multipliying triples by $k > 0$, it's no longer guaranteed.
...
Computes $C_n^k$ modulo $m$.
Precomputes factorials and reversed factorials. \tc{n}
\cppcode{math/modulo/modcomb.cpp}
